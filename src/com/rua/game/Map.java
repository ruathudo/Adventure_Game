...
public class Map {
	...
	private void loadImage(){
		this.titleImages = new HashMap<Integer,BufferedImage>();
		...
		try {
			...
			this.titleImages.put(0, background);
			this.titleImages.put(1, wall);
	...
	public void draw(Graphics g) {
		for(int row=0; row < MAP_SIZEY; row++) {
			for(int col=0; col < MAP_SIZEX; col++ ) {
				BufferedImage tile = this.titleImages.get( mapTiles[row][col] );
